...
{
    // 1. 针对 OVR 错误的特殊处理：必须读 DR 寄存器才能清除硬件标志
    if (hspi->ErrorCode & HAL_SPI_ERROR_OVR)
    {
        // 读取 SR 和 DR 是清除 OVR 标志的标准硬件操作
        volatile uint32_t tmpreg_sr = hspi->Instance->SR;
        volatile uint32_t tmpreg_dr = hspi->Instance->DR;
        (void)tmpreg_sr; // 防止编译器警告
        (void)tmpreg_dr; // 防止编译器警告
    }
    SPI_Bus_Manager *bus_manager = BSP_SPI_Get_Bus_Manager(hspi);
    if (bus_manager != NULL)
    {
